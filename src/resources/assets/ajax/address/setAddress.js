    ...
    $.ajax({
        ...
        success: function (datos){
            ...
            $.each(datos, function(index, parroquia) {
                ...
                {
                    $('#parish_id').append(
                        '<option value="' + parroquia.id + '" selected="selected">' + parroquia.desc + '</option>');
                }
                else
                {
                    $('#parish_id').append(
                        '<option value="' + parroquia.id + '">' + parroquia.desc + '</option>');
                }
            }); //fin del each
            $.ajax({
                type: 'GET',
                dataType: "json",
                url: '/api/direcciones/municipios/' + municipioId,
                data: null,
                success: function (datos){
                    ...
                    $.each(datos, function(index, municipio) {
                        if (municipioId === municipio.id) {
                            $('#town_id').append(
                                '<option value="' + municipio.id + '" selected="selected">' + municipio.desc + '</option>');
                        }
                        else
                        {
                            $('#town_id').append(
                        ...
                        success: function (datos){
                            ...
                            $.each(datos, function(index, estado) {
                                if (estadoId === estado.id) {
                                    $('#state_id').append(
                                        '<option value="' + estado.id + '" selected="selected">' + estado.desc + '</option>');
                                }
                                else
                                {
                                    $('#state_id').append(
